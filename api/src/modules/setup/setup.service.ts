...
export class SetupService {
  ...
  async save(setupDto: SetupDto) {
    if(this.isConfigFileCreated()) {
      throw new HttpException(`Please remove ${configFileName} before editing the configurations of your server!`, HttpStatus.CONFLICT);
    }
    ...
    console.log(`${configFileName} written to api`)
    return "OK"
  }

  get(): Promise<SetupDtoOut> {
    if(!this.isConfigFileCreated()){
      throw new HttpException(`${configFileName} nao existe!`, HttpStatus.BAD_REQUEST);
    }

    const config = require(`../../../${configFileName}`)
    return this.isDatabaseReady(config)
    .then(({migrationsNumber,userCount }) => {
      const dataJSON = fs.readFileSync(configFileName, 'utf8');
